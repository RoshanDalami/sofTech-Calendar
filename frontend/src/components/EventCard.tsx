...
export default function EventCard() {
  ...
  const getEventsByUser = async()=>{
    try {
      const response = await axios.get(`${url.getAllEventsByUser}/${user.data._id}`)
  ...
  const getEventOnMonthCount = async()=>{
      const eventsOnMonth = events?.filter((item)=> (item.eventDateNepali.split("-")[0] === nepali_date.format("YYYY-M-D").split('-')[0])&& (item.eventDateNepali.split("-")[1] === nepali_date.format("YYYY-M-D").split('-')[1]   ) )
  ...
  const getTaskByUser = async()=>{
   try {
    const response = await axios.get(`${url.getAllTaskByUser}/${user?.data?._id}`);
